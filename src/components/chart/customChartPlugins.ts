...
export const drawColorsFill = {
  ...
  beforeDatasetsDraw(chart) {
    ...
    for (let i = 0; i < dataPoints.length - 1; i++) {
      const currentValue = dataPoints[i];
      const nextValue = dataPoints[i + 1];

      const currentX = xScale.getPixelForValue(currentValue.x);
      const nextX = xScale.getPixelForValue(nextValue.x);

      const currentY = yScale.getPixelForValue(currentValue.y);
      const nextY = yScale.getPixelForValue(nextValue.y);
      ...
      if (currentValue.y > 0 && nextValue.y > 0) {
        ctx.fillStyle = "rgba(0, 200, 0, 0.2)";
        ctx.fill();
      } 

      else if (currentValue.y < 0 && nextValue.y < 0) {
        ctx.fillStyle = "rgba(200, 0, 0, 0.2)";
        ctx.fill();
      } 
			
      else {
        const crossingX = (currentX * Math.abs(nextValue.y) + nextX * Math.abs(currentValue.y)) / 
                          (Math.abs(currentValue.y) + Math.abs(nextValue.y));
        const crossingY = yScale.getPixelForValue(0); 

        if (currentValue.y > 0) {
          ctx.beginPath();
          ctx.moveTo(currentX, currentY);
          ctx.lineTo(crossingX, crossingY);
          ctx.lineTo(currentX, yZero);
          ctx.closePath();
          ctx.fillStyle = "rgba(0, 200, 0, 0.2)";
          ctx.fill();
